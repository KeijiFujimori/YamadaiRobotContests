...
void CamCB(CamImage img) {
  ...
  // tft.drawRGBBitmap(0, 0, (uint16_t *)img.getImgBuff(), 320, 224);                                           // 書いた画像をそのまま転送ここ大事
  // tft.fillRect(0,0, 320, 240, ILI9341_BLACK);
  
  int t = millis();       // 秒数を取っていく
  if((t-tpre)>3000){      // 目標値と起動時間の差が1秒以上という条件
    if(Result_pre == gStrResult){                        // 数値を比較 
      // putStringOnLcd(gStrResult, ILI9341_YELLOW);     // 条件達成：黄色い文字で出力
      tft.fillRect(0,0, 320, 240, ILI9341_BLACK);
      tft.setTextSize(6);
      int len = gStrResult.length();
